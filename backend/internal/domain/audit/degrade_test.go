...
func TestClassifyOutputSpeed(t *testing.T) {
	tests := []struct {
		...
		{name: "burst short window", output: 200, first: 50, duration: 250, soft: 500, hard: 1000, minGen: 1000, wantClass: DegradeClassBurst},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			class, _, _ := ClassifyOutputSpeed(test.output, test.first, test.duration, test.soft, test.hard, test.minGen)
